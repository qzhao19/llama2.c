...
void Transformer::malloc_run_state() {
    ...
    if (state_->x.empty() || state_->xb.empty() || state_->xb2.empty() || state_->hb.empty() || state_->hb2.empty() || 
        state_->q.empty() || state_->att.empty() || state_->logits.empty() ||
...
std::vector<float> Transformer::forward(int token, int pos) {
    ...
    for (unsigned long long l = 0; l < model_->config->n_layers; l++) {
        ...
        matmul(model_->state->k, model_->state->xb.data(), model_->weight->wk.data() + l * dim * kv_dim, dim, kv_dim);
        matmul(model_->state->v, model_->state->xb.data(), model_->weight->wv.data() + l * dim * kv_dim, dim, kv_dim);
